  ...
  try {
    ...
    if (from) {
      const openingSales = await prisma.sale.findMany({
        where: { clientId: id, createdAt: { lt: new Date(from) } },
    ...
    sheet.getCell("A3").value =
      from && to ? `Период: ${from} — ${to}` : "Период: весь период";
    ...
    const totals = sheet.addRow([
      ...
      closing,
      `Сальдо на конец: ${
        closing > 0 ? "долг клиента" : closing < 0 ? "предоплата" : "0"
    ...
    const safePlate = String(client.licensePlate || "client")
      .replace(/[^\w\-]+/g, "_")
      .slice(0, 40);
    const filename = `akt_sverki_${safePlate}_${from || "all"}_${to || "all"}.xlsx`;
